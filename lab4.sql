...
(select c.cus_id,cus_gender from customer c 
join `order` o on c.cus_id=o.cus_id where ord_amount>=3000 group by c.cus_id) 
...
as o join supplier_pricing as s on o.pricing_id=s.pricing_id
 join product p on s.pro_id=p.pro_id  where cus_id=2;

/* q.5 Display the Supplier details who can supply more than one product.
*/

/* q.6Find the least expensive product from each category and print the table with category id, name, product name and price of the product
 */
